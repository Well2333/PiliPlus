...
- 默认昵称标识开启时，设置框不出现 `[piliplus]` 或内部会话 ID，聊天列表只显示原昵称加后缀；关闭后
  不附加后缀且房间页不显示 PiliPlus 用户数。
- 两台同为 PiliPlus 时分别发送真实文字消息，确认计数最终与在线成员一致并自动允许双向接管；混入官方
  浏览器客户端或存在未发消息成员时，确认计数不足且继续兼容模式，不发生误抢控。
