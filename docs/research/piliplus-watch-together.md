...
本分支因此不实现客户端类型检测：不追加昵称后缀、不显示推测的 PiliPlus 用户数，也不通过聊天消息自动
绕过兼容模式。双向同步只能由用户在房间页面显式开启；仅在明确知道全部客户端都支持“最后操作者接管”时
使用，与官方浏览器插件、旧版或其他客户端混用时关闭。若未来可修改服务端，应正式增加并广播
`clientCapabilities`，并仅在全部活跃成员声明相同双向控制版本时自动启用；不再叠加聊天握手技巧。
