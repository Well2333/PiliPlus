...
2026-09-21 基于官方提交
[`3c61f12b`](https://github.com/VideoTogether/VideoTogether/commit/3c61f12b84523abed497f690754487b5c21ec6d3)
复核 Go 服务端和中文浏览器客户端后确认：官方协议不能在入房时可靠、透明地识别客户端类型。

- 服务端 `Member` 只有 `userId`、`isLoadding` 和 `currentUrl`，房间广播只包含聚合后的
  `memberCount` 与 `waitForLoadding`，不包含成员列表、昵称或成员 `userId`；
- 房主 `tempUser` 只在服务端校验写权，返回房间状态时明确移除，不能承载客户端能力；
- Go 的 JSON 解码会忽略额外请求字段，且房间响应按固定结构重新序列化，因此自定义
  `clientCapabilities` 不会被保存或广播；
- `send_txtmsg.id` 是唯一会在同房间原样广播的可扩展身份字段，但官方插件把它用作每条消息的 UUID，
  本身没有昵称系统。自动发送空消息做握手仍会让官方插件弹出“有新消息”，并可能触发语音初始化，
  不能称为透明探测；影子房间等旁路方案还会引入竞态、过期身份和额外服务端负载。

本分支因此采用保守的被动协商：开启设置后，只给真实文字消息的发送者字段追加会话唯一 ID 与
`[piliplus]` 后缀；PiliPlus 展示时隐藏内部 ID，昵称设置框也只显示原昵称。每个客户端立即登记自己，
其他客户端必须在发送过带后缀的真实消息后才计为 PiliPlus；已识别唯一会话数与服务端成员数完全一致时，
关闭“双向同步控制”的用户才自动绕过兼容模式。房间界面的 PiliPlus 用户数也使用同一集合。

这意味着两个 PiliPlus 客户端在互相看见带后缀消息前仍会保持兼容模式，这是避免把官方客户端误判成
PiliPlus 的有意降级。若未来可修改服务端，应正式增加并广播 `clientCapabilities`，并仅在全部活跃成员都
声明相同双向控制版本时自动启用；不应继续叠加聊天握手技巧。

## 产品入口要求

- 在“我的”页面头像资料区域右上方的快捷按钮组增加“一起看”入口；
- 在投稿和番剧的视频简介操作栏分享按钮右侧增加“一起看”，不再放入三点菜单；默认未入房隐藏，入房后
  强制显示，用户可关闭未入房隐藏；
- 入口打开创建/加入房间界面；
